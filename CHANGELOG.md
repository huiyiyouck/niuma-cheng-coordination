...
- **`news-l1-db` 契约升 v1.9（PM 三项拍板）** → [contracts/news-l1-db.md](contracts/news-l1-db.md)。① Q-1 定案：`processed_news` 补 `needs_context boolean`（ai 既有质量信号不再丢弃，xiaobao 列迁移待落地）；② `score_total` database 补算定案：xiaobao 轮询补算（公式留应用层单一真源，随 ai v0.2 联调落地）；③ 订正「原文语种归 raw_items.language」笔误（该列不存在）。非 breaking。影响项目：`ai`（v0.2 实现按 v1.9 写入 needs_context）、`xiaobao`（列迁移 + 轮询补算两项待办登记）。
- **`news-l1` 契约升 v1.1（新增 §服务端点与运行时坐标）** → [contracts/news-l1.md](contracts/news-l1.md)。ai · Architect（承接 ai DevOps 的结构性建议）：此前契约**登记了「调什么」（路径）却未登记「调哪儿」（host:port）**，且双侧模式开关零命中。缺口是**对称**的——ai 停用 `8100` 时 xiaobao 无信号（全靠 ai 停机前主动核实才发现；另见待跟进 16：其 prod 的 `AI_HUB_BASE_URL` 至今仍指向已停端口），反过来 xiaobao 改端口或调 `adminAllowedIps` 时 ai 亦无信号，且 KB 失败**主流程不中断、只持续降级**。更要紧的是 **`AI_INTEGRATION_MODE` / `RUN_MODE` 均未登记**，而 AC-1.5 的回滚是**有序双侧动作**（先 xiaobao 切回、后 ai 切），执行方读不到对方当前模式 → **预案不可执行**。处置照 `AI_STALE_TIMEOUT_MS` 先例：不把端口写死进字段定义（避免每次换端口都升版本、退化成又一份过期文档），而是单列一节 + 变更纪律「任一侧变更含停用须先改本节并知会」，两侧部署就绪检查各含一条核对。ai 侧四项已填（test `8102` / prod 待部署回填 / `RUN_MODE`），xiaobao 侧 `kb-search` base URL 按 CN-007 双方确认值填入、**`AI_INTEGRATION_MODE` 当前值待其回填**。非 breaking，不改任何字段。影响项目：`ai`、`xiaobao`。
