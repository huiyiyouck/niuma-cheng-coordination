...
|------|------|
| 项目 id | `workboard` |
| 名称 | 项目管理工作台（Web 端项目迭代 IDE） |
| 技术栈 | React 18 + Vite + Tailwind v4 + shadcn（前端，对齐小报）+ 本地 Node 聚合后端 |
| 仓库 | `git@github.com:huiyiyouck/niuma-cheng-workboard.git`（本地 `~/Project/niuma-cheng-workboard`） |
| 职责边界 | 读写工作台（5 视图：新增项目会话 + 角色映射 + 对话查看 + 迭代时间轴）；聚合各项目团队工作流状态、接入诊断、跨项目需求与阻塞；对被监控项目的进度文档**不回写** |
| 当前入口 | 项目内 `docs/progress/INDEX.md`（**已接入 agent-workflow @66c1e1a**；**v0.2 已上线**（2026-07-07）生产 `workboard.huiyiyou.cloud`；v0.3 进行中） |
| 关联项目 | `xiaobao` / `ai` / `coordination` 等（聚合对象） |
