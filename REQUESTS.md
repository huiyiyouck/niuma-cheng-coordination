...
| BCR-003 | workboard · PM（2026-06-24） | 项目元信息（定位/名称/技术栈/上线/接入状态）变更后，生态层真源（`PROJECTS.md` / 根 `/root/Project/CLAUDE.md` 索引 / BCR 回流清单接入状态）无同步跟踪机制 | agent-workflow `mechanisms.md` 迭代关闭检查清单 或 `cross-project-collaboration.md` PROJECTS.md 维护约定；下游同步 | 评估中 | 真源会话评估中：方向 + 执行协议成草案 v2.1 —— 元信息变更同步落 coordination `STATUS.md`、真源繁忙时「挂根 + 暂停」转交协议、根 `/root/Project/CLAUDE.md` 设计；Owner 已通过 5 个拍板点（台账位置 STATUS.md / 字段 / 挂机协议 / 根 CLAUDE.md 两节 + 调外·主从关系）。设计草案 @ agent-workflow `docs/progress/ad-hoc/2026-06-25-root-claude-md-redesign.md`（v2.1 `1b07f26`、`ced333b` 收尾，分支 `fix/source-repo-bcr-no-role-switch`，未合 main）。下一步：真源会话落 baseline 后转「已采纳 / 已落地真源」 | — | — | 触发自 workboard v0.1 定位升级 + 技术栈变更；订正根索引时发现无跟踪机制；本仓 BCR-001 回流清单 workboard「未接入」亦已过期 |
