...
| BCR-014 | workboard · PM（2026-07-13） | 入口「你是 {角色}」角色切换机制无护栏，允许「产出方会话切成 Review 方审自己的产出」= 自审，架空「两方独立 Review 防自审」门禁——纯靠执行者自律，自律会失效 | agent-workflow 入口 `CLAUDE.md`/`AGENTS.md` 角色切换触发；`multi-agent-workflow.md`/`runtime.md` Review 机制；下游各项目 | 已回流下游（终态） | 参谋长合并评估（与 BCR-016 同轮）+ 独立会话 R1 对抗评审通过 + 指挥官确认（2026-08-08）：已采纳——入口新增「Review 独立性检查」（产出方会话拒绝切 Review 角色，Owner 亲口指定不豁免）+ multi-agent Review 机制补「独立会话」硬性表述 + 回归用例 G14；机器化候选记「产出与 Review 结论同 commit」特征检测（R1·M1），留 L1 晋级后评估。方案游标 agent-workflow `docs/progress/ad-hoc/2026-08-08-bcr-014-016-guardrails-eval.md`（定稿 v2） | `9bdc949` | xiaobao `52c4463` / ai `da87d7c` / workboard `f7ee8a8`（2026-08-08） | 实例触发：workboard v0.3 R1，PM 写完 PRD 后同会话切 Architect 审自己，Owner 当场拦下。建议护栏：切换到 Review 角色前检查当前会话是否为被审产出的产出方会话，若是则拒绝/警示 + 提示新开会话冷启动。详情见下 |
| BCR-015 | 指挥官（Owner）发起 · 参谋长代拟（2026-07-25） | 组织定位升级：全链路自动化梯度 L0→L3——决策权分层下放（A 组 4 项永久保留 / B 组 9 项分级）+ 留痕对偶原则（砍停等、不砍留痕）；**本次仅授权 L1**（验收可执行化 + CI 门禁 `l1-gates` + 阶段间免确认 + fail-closed 降级条款），L2/L3 届时各自另立 BCR | **仅授权 L1**：根 CLAUDE.md 组织法则三表述；baseline 停等规则 L1 子集（multi-agent §7 :115 拆句 / mechanisms 总纲 :7 + 机制总表 :26-29 + §2§3 / runtime 新增三条 P0 红线，含 A4 生产发布显式红线）；PRD / 迭代记录 / 自测报告模板；试点仓 workboard CI（`l1-gates.yml`，Owner 拍定） | **已落地真源（U1-U3 全批次）· L1 试运行阶段** | 方案三份 v3 定稿（R1/R2 两轮对抗评审收口，评审方过目通过）：agent-workflow `docs/superpowers/specs/2026-07-25-fullchain-automation-bcr-draft.md`（BCR 定稿）/ `2026-07-25-fullchain-automation-program-design.md`（整体走法：阶段 0 + U1-U3 批次 + 晋级三关）/ `2026-07-25-l1-stage-automation-design.md`（L1 方案：S1-S4 停等处置 / G1-G5 门禁 / 降级条款） | U1 `62d8c8c`（PR #10）；U2 `8eacb03`+`6cd4e95`（PR #11+修正，framework-checks 全绿）；U3 `747f058`（免确认+降级条款+A4 红线，防误伤 6 红线自检过）——**U1-U3 全部合 main（2026-07-26）** | **三仓全部对齐 `5e289fb`**（U1-U3 + U3.1 销项）：workboard（门禁 v2 首跑绿 run 30240606527）/ xiaobao / ai。**R3 落地复核已收口**：1 阻塞 3 高（门禁网薄于宣称）全部修复——G2 逐行对应+取值枚举、G5 双逃逸加固、Change Note 终态、真回放定性归档（误报 0）、收窄面逐项报备；治理拍子补「落地复核」环。**U3.2 `2f3fc3d`（2026-07-27）：指挥官销项复核实证的 escA/escB 双逃逸已修**（closing 并取最终状态 + 关闭态报告缺失即 fail，fixtures 1 正 12 负，workboard CI 绿 run 30248897391；fixture 待指挥官抽验销项）。**试运行放行条件已满足** = workboard 下一真实迭代（覆盖矩阵三项）；晋级三关：残留 ≤2 + 注入回放 + Owner 留痕回溯。终态待晋级评审通过后置 | 0.2 台账已完成并经 Owner 拍板（2026-07-26「按台账定性通过，阈值 ≤2 认可」）：真停等 47 落点/33 规则、L1 砍 6 条、L0 基线 workboard S1-S4 ≈7 次/迭代 → 晋级绝对阈值残留 ≤2；台账 agent-workflow `docs/progress/ad-hoc/2026-07-25-bcr-015-stop-point-ledger-l0-baseline.md`。U1（模板与留痕）已落地回流；下一批 U2 CI 门禁。**R3 落地复核（2026-07-27，独立评审会话）：规则层 U1/U3 合格、防误伤六红线全在；门禁层实证缺口 1 阻塞 3 高（G2 缺行/红状态放行、G5 打回闸双逃逸、mechanisms 断言的 CI 承接两项空头、存量回放空心化），建议修复后再放行 workboard 首个免确认迭代（是否带险并行归 Owner 裁量）——详见 L1 方案 Review 记录 R3**。**R3 销项确认（2026-07-27，评审会话逐项复核 `5e289fb`）：11 项全部销项成立、五处报备收窄逐项认可；新发现关闭态判定 2 条边界逃逸（最终状态直书已完成绕过严格核对 / 关闭态缺自测报告零检查，均中级、实证），建议随手补完这枚小补丁再放行——详见 R3 销项确认小节**。**~~指挥官改拍（2026-08-08，参谋长会话拍板留痕）：L1 试运行首试点由 workboard 改为即将新立的项目（id 待立项后回填），新项目自 Bootstrap 起即带 `l1-gates` + L1 免确认；workboard 降为后续试点；因新项目无 L0 停等基线，晋级判据调整方案由参谋长随立项另出、报指挥官拍板~~ → **已撤销。指挥官二次改拍（2026-08-08 同日晚）：取消新项目立项；改为「工作流迭代（L1+L2 框架改造）全部完成后，再实施到存量下游项目」。** 故：① 试运行/实施载体**回到存量三仓**，首试点仍为 **workboard**（13 天/迭代节奏最稳、已有测试基建与 `l1-gates`、解析侧度量同仓）；② **晋级判据回到 0.2 台账原口径**——workboard L0 基线 S1-S4 ≈7 次/迭代、残留 ≤2 已经 Owner 2026-07-26 拍板认可，**无需另出调整方案，参谋长该项待办随之取消**；③ 立项七要素与建壳动作取消，参谋长职责①本轮不触发**。**⚠️ U3.2 收官结论订正（2026-08-08，BCR-014/016 评估 R1 揪出 B1）：`c0dc32a`「双逃逸修复收官、试运行放行条件满足」在 CI 实际平台（GNU）上当时不成立**——after_colon 字节偏移缺陷致 G5/关闭态判定在 Linux 静默 fail-open，真源 framework-checks 自 07-27 持续红且历轮回执只看了 workboard 绿灯；**已修 `f7dff26`（+selftest 双语义防复发），framework-checks 转绿、fixtures 双平台全绿，放行条件此刻方真正成立**。详情见下 |
| BCR-016 | ai · DevOps（2026-08-01） | 多角色共享同一 git 工作区，`git add -A` 无护栏防「卷入他人未提交改动」——现有 [P0] 只要求事后核对 stat，实测防不住；建议提前为事前限定范围（默认禁裸 `git add -A`） | agent-workflow Git 安全规则（`conventions.md` 等 Git 章节）；回流后影响各下游所有角色的提交行为 | 已回流下游（终态） | 参谋长合并评估 + 独立会话 R1 对抗评审 + 指挥官确认（2026-08-08）：**按 R1·G1 修订采纳——一刀切全禁裸 `git add -A`/`git add .`，不留例外通道**（原「批量暂存例外」被否决：例外判据复用已失效的事后核对机制且批量场景论据被 sync 白名单实况证伪）；批量场景列固定路径清单，判据「命令里没有 `-A`/`.`」机器可查。git hook 硬拦截不做（暂存方式 git 层不可追溯）。方案游标同 BCR-014（定稿 v2）。**R1 另揪出阻塞级前置 B1**：`l1-gates.sh` after_colon 按字节跳全角冒号，GNU awk 字符语义下取值截断 → G5/关闭态判定静默 fail-open、真源 CI 自 07-27 持续红；已修（agent-workflow `f7dff26`，shell 参数展开 + `--selftest` 双语义防复发），framework-checks 首次转绿 | `9bdc949`（B1 前置 `f7dff26`） | xiaobao `52c4463` / ai `da87d7c` / workboard `f7ee8a8`（2026-08-08，workboard 含 B1 门禁修复） | **同日两起、方向相反**（DevOps 卷入 Architect；PM 卷入 DevOps），故非个人疏忽。详情见下 |
...
## BCR-015 · 组织定位升级：全链路自动化梯度（仅授权 L1）

- 提出方：指挥官（Owner）发起，参谋长会话代拟（2026-07-25）。
- 性质：**宪法层改动**——根 CLAUDE.md 组织法则三预留的「下一次定位升级：全自动（决策权下放）」正式启动；Owner 直接发起，评估 · 落地由参谋长执行，重大取舍逐项报 Owner 拍板。
- 定位陈述：给一个目标，团队自动按目标做完、输出交付物（goal in → deliverable out）；生态层面项目间协调也自动化。分四级梯度渐进（L0 现状 → L1 阶段内自动 → L2 整迭代自动 → L3 跨项目自动协调），**一次 BCR 只锁一级，本次仅授权 L1**。
- 核心机制：决策权分类（判据「改变『什么算做好了』→ A 组永久保留；只决定『怎么把它做好』→ B 组分级下放」+ 存疑向上取整）；留痕对偶原则（每下放一项决策权必须同步新增对应留痕义务——砍停等、不砍留痕，介入减少、留痕增厚）；fail-closed 降级条款（约定名门禁 `l1-gates.yml` 非绿灯结论一律不自动推进，维持 Owner 确认并登记降级留痕）。
- 方案真源（三份 v3 定稿，R1/R2 双轮对抗评审收口）：agent-workflow `docs/superpowers/specs/` 下 `2026-07-25-fullchain-automation-bcr-draft.md` / `2026-07-25-fullchain-automation-program-design.md` / `2026-07-25-l1-stage-automation-design.md`。
- 试点仓：`niuma-cheng-workboard`（Owner 2026-07-25 拍定，首批只搭该仓 CI）；第二试点 ai（待恢复迭代）；xiaobao 最后进且先补测试基建（L1 范围外独立前置任务）。
- 下一步：评估阶段产出——停等点分类台账（78 行筛查底表逐条定性 + 敏感条目单列逐条拍）与 L0 分仓基线实测（落 agent-workflow `docs/progress/ad-hoc/`），随后 U1-U3 批次落地、试运行、晋级评审。
- 关联：前案《agent-workflow 升级定位与实现方案》已废弃（Skills 化搁置至 L2 按需重估；调研存档见生态根 `调研/TRAE-AI通识方法论/`）；判例对照——BCR-006（L1 验收可执行化是其演进非回退）、BCR-013（度量走解析侧、零框架税）。
---

## BCR-016 · 多角色共享工作区，`git add -A` 无护栏防「卷入他人未提交改动」

- 提出方：ai · DevOps（2026-08-01）
- 问题：一人公司形态下多个角色会话**轮流在同一个 git 工作区**工作。角色 A 提交自己的产出时若用 `git add -A` / `git add .`，会把角色 B 尚未提交的工作区改动一并提交，而 A 的 commit message 完全不涉及那部分——造成**归属错乱与审计线断裂**。现有 **[P0]**「标注 AI 协作信息的 commit，push 前贴 `git diff --stat` 核对；与 message 范围不符则停等 Owner」**只要求事后检测、未限定事前范围**，实测防不住。
- 实例触发（**同日两起，方向相反，故非个人疏忽**）：
  1. ai DevOps 提交 CN-009 确认（ai 仓 `a2943e2`）时用 `git add -A`，卷入 Architect 尚未提交的 `v0.2-design.md`（24 行，CN-009 变更 1 的设计侧落地）。执行者**确实跑了** `git diff --cached --stat`、输出里也有该文件名，但只扫了一眼未逐项核对。
  2. 同日稍后，ai PM 会话提交（coordination `0cc7510`）时**反向卷入** DevOps 已写好未提交的 REQ-003 回帖（约 40 行）。
  两起均已 push，[P0] 禁 force push 故无法改写历史，只能补说明留痕。
- 建议方向（供 Owner + 参谋长评估，不定死）：把提交范围从「事后核对」**提前为「事前限定」**——Git 安全规则中明确：**多角色共享工作区默认禁止裸 `git add -A` / `git add .`，一律 `git add <明确文件列表>`**；确需 `-A` 时，`git diff --cached --stat` 的**每个文件名须逐一匹配本次改动清单**，判据是「文件清单逐一匹配」而非「有没有报错」。
- 理由：现有规则的失效点在于**它检测的是「范围是否相符」这一主观判断，而人在收尾时倾向于快速通过**；改成「只 add 自己列出的文件」则**从机制上不可能卷入他人改动**——把自律要求换成操作约束。**与 BCR-014 同型**（靠自律守的门迟早破，需机制护栏兜底）。
- 补充观察：该问题的暴露概率随「并行角色数 × 会话切换频率」上升，而 BCR-015 的 L1 阶段内自动化会**提高**这两个量——建议评估时一并考虑二者的相互作用。
- 影响范围：agent-workflow Git 安全规则（`conventions.md` 等 Git 相关章节）；回流后影响各下游所有角色的提交行为。
- 落地：本提案仅提报，评估 · 采纳 · 落地由 Owner + 参谋长（框架维护权 BCR-009）。
---
