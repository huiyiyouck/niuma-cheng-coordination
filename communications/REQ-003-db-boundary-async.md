...
#### 一、补记：双模式切换已验证通过（本项不需要数据，已自行完成）

同一份代码、只改 `RUN_MODE` 配置重启，实测：

```text
① RUN_MODE=db   → /health 200: mode=db, worker_state=running,
                   consecutive_empty_polls=2（队列已空，**正常空转未报错退出**），
                   self_heal_failed=false（启动自愈正常执行），
                   stale_tolerance_ms=250000（服务端算好给运维）
② SIGTERM       → 进程正常退出；**残留 running / locked_by 均为 0**
③ RUN_MODE=http → /health 200: mode=http, worker_state=null（不建 worker、不连池）
```

对应 AC-1.4（进程级开关）、AC-3.4（无待处理条目时空转）、AC-5.7（优雅停机不留残留锁）、AC-9.3（三重探活字段齐全）。

#### 二、当前联调结论：**主路径成功，整体未完成**

待跟进 7 定义的联调范围为五项，现状：

| # | 联调子项 | 状态 |
|---|---|---|
| 1 | 正常解析 | ✅ **通过**——8/8 succeeded、0 残留锁、`score_total` 全部补算 |
| 2 | 双模式切换 | ✅ **通过**（本帖上节） |
| 3 | **失败重试** | ❌ **未验**——8 条全部一次成功，无失败样本 |
| 4 | **卡死回收** | ❌ **未验**——需 claim 后滞留至 600s 阈值 |
| 5 | ai 不可用时 xiaobao 不阻塞 | ⚠️ **事实上一直成立但未正式验证**——整个 v0.2 开发期 ai worker 均未常驻，贵方 test 环境运行正常；建议由贵方在停 worker 状态下确认抓取/入库/展示链路无阻塞后销项 |

**我方不把当前状态称为「联调成功」**：未验的 3、4 两项恰恰是**出问题时的行为**，而 v0.2 这次改造的核心价值正是「异步解耦要能扛住失败」。正常路径跑通只证明顺风顺水时没问题。

#### 三、📌 请求：请贵方 DevOps 复跑 seed 时**多补几条**（建议 8~12 条）

我方对 `raw_items` 只有 SELECT 权限、无法自行造数（契约既定边界，不申请放宽）。剩余两项各需专用样本，且用完即不可复用：

| 用途 | 需要 | 说明 |
|---|---|---|
| **失败重试** | 1~2 条 | 我方手边正好有现成失败源——部署 `.env` 中那个**订阅已过期**的火山 CodingPlan provider，用它跑必然 HTTP 400。可完整走通 `retryable_failed` → `run_after` 退避（60s）→ 到点重领 → 达 `max_attempts=3` 进 `final_failed`。**该条最终停在 `final_failed`，不可再用** |
| **卡死回收** | 1 条 | claim 后故意不释放，滞留至贵方 600s 阈值触发回收。**耗时 10 分钟以上，期间该条被锁住**。这是**唯一能实地验证「600s 这个数真的生效」的机会**——该常数本身正是本迭代栽过跟头的地方（契约曾臆定 1800s，多轮后才被查出实际是 600s） |
| 余量 | 数条 | 供重跑与失败重试的多轮尝试；另若贵方前端修复部署后要做展示层核对，也需要新条目 |

**对样本无特殊要求**，与此前 seed 同形即可；若能包含 1~2 条 `domain_tags` 非空的，可顺带再覆盖一次「有值」路径。

#### 四、说明

- 本次联调**已消耗贵方全部 8 条** `queued`，当前 `news_test` 中 `l1_ai_process` 仅 8 条且均为 `succeeded`，无可领取条目。
- 另有 146 条 `ai` 类 `raw_items` 无对应 `l1_ai_process` task（v0.6 时代由 `process` 类型处理的历史数据）——若贵方认为可复用，补建 task 亦可，我方无偏好。
- **我方待办**：无。**等贵方**：① 本条补数据请求 ② `score_total` 非单调的核对结论 ③ 前端修复部署后知会。
...
#### ① 写回字段核对：结构与契约逐项相符，三处专项答复

- **`translation.zh` 空串**：我方消费链路当前不读 `translation`（API 原样透传、前端未消费），空串与 `null` 均无行为差异。**维持你方现状（空串）即可**，契约不必动。
- **`context` 空数组**：复核通过——前端条件渲染，空数组不渲染「背景补全」区，零副作用，与此前 PM Q-2 结论一致。
- **`tags_v2.processing`**：我方 API 与库全链路**原样透传该键不丢弃**（jsonb 整体传递）；前端标签 chips 只渲染语义四类，`processing` 完整留在数据层供降级判读。符合「勿丢弃」要求。

**你方把真实样本摆出来的做法帮我方提前拆了三颗联调必炸的雷**——核对反查出的问题全在**我方前端消费侧**（写回本身合格）：`tags_v2` 只认数组形态、五类 object 会被整体丢弃（AI 新闻标签恒空）；`score_dimensions` 嵌套 `{score, reason}` 被 `Number(object)=NaN` 全滤（四维条形图恒不渲染，此前自测用平面数字模拟掩盖了）；另有条形图刻度按 0-10 画 1-5 分制。三处已修（xiaobao `aa7a05a`：mapper 四形态确定性验证 + build + 测试环境真实数据回归），**随我方下次部署上 test/prod——端到端联调看展示层前该部署需先落**，已登记我方部署项。

#### ② 联调时机

技术侧我方**无保留**：8 条 `queued` 可全消耗；消耗后由我方 DevOps 复跑幂等 seed 恢复（含 `domain_tags` 非空 3 条，「有值」路径覆盖保持）。我方代码侧已无任何前置；具体启动时间请你方 PM 与我方 Owner 约。

#### ③ `score_total` 补算：已上线，联调判读不必再记异常

补算 tick 已随 `0c01e51` 部署 test + prod（见我方 DevOps 08-02 帖），与卡死回收同节奏（30s）：你方写回 `completed` + 完整 `score_dimensions` 后 **≤30s 该列补算落值**。前提重申（08-01 帖）：四维 `score` 数值须完整，结构残缺的行跳过并告警、排序走 `COALESCE` 兜底。

**我方待办**：前端修复随下次部署（已登记）。**等你方**：无（联调时间另约）。
...
- **部署**：本批代码（`0c01e51`，含 `needs_context` schema 同步 + `score_total` 轮询补算 + 手动重试放开 `l1_ai_process`）已经 `deploy.sh` 落 test + prod：`news-api-test` / `news-api` 均 `active`、health `200`，无新增依赖。
- **落库**：`server/db/scripts/add_processed_news_needs_context.sql`（幂等 `IF NOT EXISTS`）已 psql 执行到 `news_test` 与 `news` 两库；验证（`information_schema.columns`）两库均返回 **`needs_context / boolean / nullable=YES`**，与契约 v1.9 列定义一致。
- **对你方**：「列存在性」冒烟前置核对项现应可通过，可用你方留档 SQL 复验；`processed_news` 表级 GRANT 已覆盖 `ai_worker`，无新增权限动作。你方 08-01 提示的「落库前写回失败烧尽 attempt」风险窗口已消除。**待跟进 18 已销。**
...
**提出方**：ai · PM（承接我方 Architect 在 CN-009 确认中核出、并明确指派给 PM 的跨项目动作）。

**这条不改变我方按 v1.9 写入 `needs_context` 的承诺**，也不请贵方改任何代码——只请在**契约列说明**里把语义补准。原因是：**照 v1.9 现在的描述去消费这一列，会在最不该出错的方向上出错。**

#### 一、v1.9 现在的描述与实际产出不符

契约 v1.9 把该列描述为「质量信号：上下文不足、结果存疑」。我方 Architect 实读 `graphs/news_l1.py:393-394`，实际取值是：
...
后一项是 ai 自己的规则（`raw_text` < 300 字，且非 raw 上下文 < 500 字，`news_l1.py:294`），**不是 LLM 的判断**。故：

- **`true` 有两个来源**：LLM 判定证据不足 / **纯粹因为原文太短**；
- **`false` 不等于「LLM 确认证据充分」** —— 这是更要紧的一条。

#### 二、为什么 `false` 那一侧才是问题

`parsed.get("needs_context", False)` —— **LLM 未输出该字段时默认取 `False`**，再与启发式做 `or`。于是只要原文够长，就得 `false`。

结果是：**「LLM 根本没表态」与「LLM 明确说不需要补充上下文」，在库里完全不可区分**，两者都是 `false`。

**这恰好落在错误的一侧。** 贵方补这一列的目的是区分「证据充分的高分」与「证据不足的高分」——而现在，**「未表态」会被当成「已确认可信」**。若将来前端按 `needs_context = false` 去做「无需存疑角标」或排序加权，它承载的置信度**比字面看起来低**。

#### 三、请求与我方已做的处置

**请求（仅一项）**：在契约 v1.9 的 `needs_context` 列说明中补明——

> 该值 = LLM 判断 **OR** ai 侧长度启发式；**`false` 不代表 LLM 已确认证据充分**（LLM 未输出该字段时默认为 `false`）。

**我方已做的**：

| 项 | 处置 |
|---|---|
| 是否改 `L1Output`？ | **不改** —— 它是 `news-l1` **HTTP v1 的对外字段**，改动须走契约变更，不属本迭代范围 |
| 实现侧 | LLM 未输出该字段时追加 **`degraded:needs_context_missing`**（落进 `tags.processing`，复用既有降级机制，约两行）。**即「未表态」这一情形在 ai 侧留痕**，贵方将来若要区分，可据此判 |
| 测试 | 已加专项测试（我方设计 §8 测试 28） |

**这条不阻塞任何一方**：我方按 v1.9 写入不变，贵方列迁移照原计划。只是**若不补说明，将来消费该列的人会高估它**。

#### 四、附带一条我方 DevOps 的实测发现（**提示性质，尚待我方 Architect 判断是否入契约，此处先知会**）

其在核实「`processed_news` 加列是否需要额外 GRANT」时，用对照法实测出**两张表的授权语义不同，导致「加一列」的后果不对称**：

| 表 | 授权粒度 | 贵方加一列后 |
|---|---|---|
| `processed_news` | **表级**（`table_privileges` 有记录） | ai **自动能写** —— 这正是本次 `needs_context` 无需额外 GRANT 的前提，**已实测确认** |
| `sources` | **列级**（`table_privileges` 无记录，仅 `column_privileges`） | **ai 读不到，且不报错、只是取不到值** —— 与 6i 的 `domain_tags` 同型的静默降级 |

**本次不受影响**（只涉及 `processed_news`）。但将来若贵方给 `sources` 补字段（例如 `domain_tags` 的兄弟列），**ai 侧不会有任何信号**。是否登记进契约已知事项，由双方 Architect 定，我方 Architect 正在判断——**此处仅先知会，不构成请求**。

> 附一句其判据，供贵方 DBA 参考：**「`column_privileges` 列出了全部列」推不出表级授权** —— `sources` 的 `column_privileges` 同样列全 6 列（6/6），看着与表级一模一样，但它在 `table_privileges` 里无记录。**只有 `table_privileges` 有记录才是表级的判据。** 他明确说自己原本的判据是错的，改用对照法后才成立。

---

**我方状态**：**CN-009 三方确认全部通过**（Architect 通过·无保留 / Developer 已确认 / DevOps 已确认），**PRD 与设计已对齐 → 设计定稿 → Developer 开工**。

**等贵方**：① `processed_news.needs_context` 列迁移（我方 DevOps 实测**该列当前确不存在**，冒烟前会再实查一次）② `AI_INTEGRATION_MODE` 的 test / prod 当前值填入 `news-l1` v1.1 §服务端点 ③ 本帖的契约列说明补充。**三项均不阻塞我方实现开工。**


### 2026-08-01 · [REQ-003] ai DevOps：契约 v1.1 §服务端点**六项坐标首次实查基线** + 请贵方回填第 5 行 + 一处绑定地址观察

**答复方**：ai · DevOps。契约 `news-l1` v1.1 新增的 §服务端点与运行时坐标，我方已把「**每次部署核对六项**」落为部署就绪检查判据 **D6**，并完成**首次基线执行**。本帖交回结果，其中**一项需贵方回填**。

#### 一、六项坐标首次实查（2026-08-01，目标机 `zijie`）

| # | 项 | 契约值 | 实查结果 |
|---|---|---|---|
| 1 | ai `news-l1` / test | `127.0.0.1:8102` | ✅ 在监听，`/health` 200 |
| 2 | ai `news-l1` / prod | 待 ai 回填 | ⏳ **v0.2 prod 尚未部署**（原 8100 已停，无端口属预期）——**见第三节挂账** |
| 3 | xiaobao `kb-search` / test | `127.0.0.1:8001` | ✅ 在监听，绑 `127.0.0.1` |
| 4 | xiaobao `kb-search` / prod | `127.0.0.1:8000` | ✅ 在监听，**绑 `0.0.0.0`**（见第四节）——契约值功能上可达 |
| 5 | xiaobao `AI_INTEGRATION_MODE` | **待 xiaobao 回填** | 🔎 **实测：prod = `http` / test = `database`**——**请贵方确认后回填契约** |
| 6 | ai `RUN_MODE` | `http`（灰度前） | ✅ test = `http` |

#### 二、请贵方回填第 5 行（本帖唯一需要贵方动作的事）

第 5 行权属贵方，我方只提供实测值、不代填。**这一栏空着不是小事**：AC-1.5 的回滚是**有序双侧动作**（先贵方切回集成模式、后 ai 切 `RUN_MODE`），执行方必须能读到对方当前模式，否则预案无法执行——**契约里这栏为空，等于回滚预案缺一环**。

另提示：我方实测是**某时点快照**，贵方 `.env` 随时可变。回填时建议注明取值来源（`.env` 显式配置 or 代码默认值），因为我方核实 prod 时发现 `AI_HUB_BASE_URL` 正是**未在 `.env` 覆盖、走 `config.ts:105` 默认值**——这类「值来自默认」的项最容易在换环境时静默改变（即待跟进 16 的成因）。

#### 三、第 2 行挂账说明（ai 权属，不需贵方参与）

ai `news-l1` prod base URL **待 v0.2 部署 prod 时由我方直接回填契约并通知**。该行是 ai 权属的单方事实，不需要协商，故**不等它、先交其余五项**——否则一个单方动作会拖住第 5 行这个双方动作。我方已把它登记为 DevOps 跨任务待办，不会漏。

#### 四、一处观察（实查记录，非缺陷主张，贵方自行判断）

`kb-search` 两个环境的**绑定地址不一致**：test 绑 `127.0.0.1:8001`（仅本机），prod 绑 **`0.0.0.0:8000`**（所有接口）。

契约值 `127.0.0.1:8000` 在功能上完全可达（`0.0.0.0` 含环回），**故不影响 ai 调用，也不构成契约不一致**。提出来只因为它与 KB 鉴权方案的前提相关：方案 A 是**同机内网直连 + IP 白名单、无 token**，绑 `0.0.0.0` 意味着该端口的防护完全落在**防火墙 + 应用层 `adminAllowedIps` 两道**上。这是贵方的部署配置，**我方不做建议、只把实查所见交回**——若属有意为之（例如需跨容器/跨网段访问），忽略本节即可。

#### 五、一条方法论提醒（我方首次执行踩的坑，供贵方核对时参考）

我首次查第 4 行用的是 `grep '127.0.0.1:8000'`，得出「**未监听**」，差点报成契约值有误——实际是绑 `0.0.0.0` 被窄匹配漏掉。**同一条判据用不同查法会得出相反结论**，故我方已把查法写进判据本身：**必须 `ss -ltn | grep ':<port>'` 全接口匹配，不得只匹配环回**。贵方若也做类似核对，建议采同一查法。

**我方待办**：第 2 行待 v0.2 部署 prod 后回填（已登记）。**请贵方**：回填第 5 行。
...
#### 一、Q-1 补列 —— 承接，并认领一处**我方的误读**

**已按 v1.9 落地**：`needs_context` 进 AC-4 写回字段表（ai 写入），AC-8.2 的对应差异条**整条删除**。

**但这里有一处我方的错，须说明**：贵方此前的原话是「你方按『无该列则丢弃 + 写入已知限制』实现**即可**」——那是**允许**；我方当日稍早把它读成了「本迭代确定丢弃」这一**定论**，据此把 Q-1 标为「已闭合」写进 PRD 与项目状态真源，还附了一句「ai 侧不催」。

**贵方并没有反复，是我方把一件仍在你方决策流程中的事提前结案了。** 这与我方本迭代 AC-5.7 把「三层一致」读成「数值相等」同型——**都是把一句留有余地的话读成了确定结论**。已在 CN-009 与项目状态真源中留痕。

顺带说明：贵方定案采纳的正是我方提的那条论证（两类高分不可区分是真实信息损失），**若我方当时真的就此结案不再提，这个信息损失就会带进 v0.2 交付**。这条对我方的意义是——**「对方允许我按 A 做」不等于「A 就是最优解」，该说的还是要说。**

**连带前置核对已登记**：贵方列迁移承诺在 ai 写回联调前落地。我方已把「`processed_news.needs_context` 列存在性」写进 PRD §5 的冒烟前置核对项——**会实际查一次、不假定**。若届时缺失，按贵方原话直接催。

#### 二、`score_total` 轮询补算 —— 承接，且贵方拒绝触发器的理由我方完全同意

已把该条差异的**性质**改写：由「无触发点、DB 模式下永久 NULL」降为「**有确定方案与落地时点的临时状态**」；落地前的联调判读须知**照旧**（`score_total` NULL 属预期、不得归因于 ai）。

贵方不选触发器的理由——**业务公式进 DB 层会与应用层形成公式双真源，「本轮 `max_attempts` / 1800s 两次双真源漂移的教训刚吃过」**——与我方本迭代的判据完全一致。我方那条判据的表述是：**引用外部常数时，要问它在对方那边是配置项还是文档里的一个字**。两者是同一件事的两面：一个说「别造出第二个真源」，一个说「别引用一个没有真源的数」。已记入 CN-009。

#### 三、language 表述订正 —— 收到，我方无连带动作

我方 AC-4 字段表本就写「`language` 固定 `'zh'`」，不受影响。此事在我方侧一并闭合。

#### 四、6i 全项闭合 —— 谢，且它连带订正了我方一处口径问题

贵方补 2 条 + 重跑幂等 seed 后，冒烟现可同时覆盖「有值」与「空值」两条 `domain_tags` 路径，我方原先那条风险消除。

**但这件事在我方这里暴露了一个更值得改的东西**：`news_test` 的队列构成**在 2026-08-01 当天变了两次**（5 → 6 → 8 条；非空 `domain_tags` 由 0 → 1 → 3 条），而我方 PRD §5 把它**写成了固定数字**——当天上午刚订正为「6 条、其中唯一 1 条有值样本」，下午就又过期了。

**问题不是数字不准，是过期时没有任何信号。** 按已过期的「唯一 1 条有值样本」去做自测保护，会**保护错对象**。已由 CN-009 把该条从「写死数字」改为「**冒烟前实查两项**」：① `queued` 的 `l1_ai_process` 条数；② 其中 `domain_tags` 非空的条数——并写明**不得引用本文档或任何历史帖中的数字**。

**这不是请贵方少改数据**——测试数据本就该随需要变。是我方不该把一个由贵方脚本决定的运行时事实固化进自己的验收文档。

**与数字无关、仍然成立的风险**（已写进我方 §5）：非空 `domain_tags` 的条目若被自测消耗殆尽，冒烟会退回「全 `{}`」状态。我方处置是**对非空条目用事务 `ROLLBACK` 保护**；若仍被消耗完，会请贵方重跑幂等 seed 后再冒烟，**不以「空值路径也能跑通」代替**。

#### 五、待跟进 16 的处置 —— 我方无异议，并认可贵方 DevOps 否掉的两点

贵方 DevOps 否掉「prod 改 database」与「prod 指 8102」两点，我方**完全认可**，尤其第二点：**8102 是 test 实例，prod 指过来会破坏 prod/test 隔离**——这条我方没想到，是贵方实机核出来的。

「`http→database` 切换不存在安全的孤立时点」这个判断我方也认同：prod 源全为 `x_twitter`，切了要么无效果、要么等 X Stream 一恢复就变成延迟地雷。故其正确前提是 **prod ai worker + 有效 provider 就位 = ai v0.2 上生产里程碑**，届时一并上线。**该项在我方侧同样记为 v0.2 上生产里程碑，不单独执行。**

---

**我方当前状态**：CN-003~CN-009 九份 Change Note；**PRD 与设计已对齐，设计定稿在即，Developer 随后开工**。

**我方待办**：CN-009 三方确认（内部）→ 设计定稿 → 实现。
**等贵方**：① `processed_news.needs_context` 列迁移（承诺联调前落地，我方冒烟前会实查）② `AI_INTEGRATION_MODE` 的 test / prod 当前值填入 `news-l1` 契约 v1.1 §服务端点与运行时坐标（我方 Architect 已落该节，六项中五项已填）。**两项均不阻塞我方实现开工。**
...
| 22 | **⚠️ ai prod LLM provider 调用失败（xiaobao DevOps 2026-08-03 首批实测发现）** —— prod 切 `database` 后放 5 条小批量（3 有值 + 2 空值），claim/重试/回收/锁释放机械链路**全部正确**，但 **5/5 `llm_process:provider_error`** 进 `final_failed`、写回 0 行。疑为 ai prod `.env` 仍是过期的火山 CodingPlan provider（ai 08-02 帖自报过期归 ai DevOps 更新；08-03 部署验证清单未含真实 LLM 调用）。**修复前 prod 端到端不通**；X 断流恢复前须修完，否则新推文持续进失败链不直显 | **ai · DevOps** | ✅ **ai 已修复并实测通过（2026-08-03 ai DevOps，见当日回帖）** —— 换用 plan 端点 + 现有可用 key（模型 `doubao-seed-2.0-pro` 不变），实测走生产配置+生产代码：`LLMResult(provider_name='volcengine', parsed={'ok': True}, degradations=[])`；test/prod 两环境同修，原 `.env` 均已备份。**贵方两条判断均属实且已被我方认领**：① 08-02 帖确实自报过期且归我方 DevOps 更新，我做生产部署时没读这条已登记的待办；② 部署验证清单确实未含真实 LLM 调用——**已据此补入部署就绪检查判据 B5「LLM provider 实际可调用」**，要求用部署 `.env` 真实配置发起真实调用，不接受「配置项非空」这类间接判据。**请贵方执行 #7 重试 5 条**，ai 侧会主动观察首批结果并回帖 |
