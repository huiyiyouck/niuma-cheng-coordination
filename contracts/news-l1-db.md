...
- 契约 id：`news-l1-db`
- 版本：v1.11（2026-08-02 xiaobao Architect：**`needs_context` 列说明语义精确化**——承接 ai Architect 核出、ai PM 08-01 转达的请求（待跟进 19）：该值 = LLM 判断 OR ai 侧长度启发式，`true` 有两个来源；**`false` ≠「LLM 已确认证据充分」**（LLM 未输出该字段时默认 `False` 再与启发式 `or`，「LLM 未表态」与「LLM 明确判定不需要」在库内同为 `false` 不可区分）；登记 ai v0.2 的 `degraded:needs_context_missing` 留痕机制为「未表态」判别手段 + 消费红线「`false` 只可当未见存疑信号用」。非 breaking，不改字段与授权）。v1.10（2026-08-01 ai Architect：① **新增 §授权粒度的非对称性** —— `processed_news` 表级授权下「加列 ai 自动可写」（CN-009 成立的前提），而 `raw_items`/`sources`/`tasks` 列级授权下「加列 ai 读不到且不报错」，两条纪律登记在案；来源系 ai DevOps 在 CN-009 确认时的提示，交 Architect 判断后判定应入。② **权限矩阵 `sources` 行补录 `domain_tags`/`attention_level`** —— GRANT 于 2026-07-28 双库执行 verify，矩阵此前漏更新，属矩阵与实际不一致的实例，已同时写入纪律 2）。v1.9（2026-08-01 PM 三项拍板落档：① **Q-1 定案补列**——`processed_news` 新增 `needs_context boolean`（ai 写入，L1Output 既有质量信号不再丢弃；xiaobao 侧列迁移待落地、前端消费留后续迭代）；② **`score_total` database 模式补算方式定案：xiaobao 轮询补算**——挂现有 worker tick，条件 `l1_status='completed' AND score_dimensions IS NOT NULL AND score_total IS NULL`，公式复用应用层 `calcScoreTotal` 单一真源（不进 DB 层触发器，避免公式双真源），实现随 ai v0.2 联调启动落地，此前排序由 `COALESCE(score_total, importance_score)` 兜底；③ **订正 PM 2026-07-25 帖「原文语种归 raw_items.language」表述——该列不存在**，原文语种当前不落任何列，`processed_news.language` 恒 `'zh'`（占位行由 xiaobao 写入，ai UPDATE 保持即可）。见 08-01 PM 帖）。v1.8（2026-08-01 Architect：① **`ALTER ROLE` 执行方改为 ai（方案甲）**——角色级 `statement_timeout`/`lock_timeout` 实际生效值为 ai 的 `4s`/`3s`（比 v1.7 文档写的 30s/5s 更严，以 ai CN-008 为准），`idle_in_transaction_session_timeout=60s` 保留 xiaobao 值并**定性为跨项目约定上限**（ai 可更严不可放宽，放宽须先改契约）；xiaobao 撤回自行执行计划。② **更正 v1.7 一处不准确表述**——`ALTER ROLE` 对 `USERSET` 参数**不是强制手段**（应用层 `SET` 随时可覆盖），其真实价值是「忘记 SET 时的兜底默认」。③ **`AI_STALE_TIMEOUT_MS` 升格为跨项目契约参数**——它约束 ai 的批量上限不变式（当前 600s 下余量仅 1.37 倍、`N=1` 唯一合法、单条预算上调空间仅 337s），任一侧变更前须先改契约并通知对方。见 2026-08-01 三帖）。v1.7（2026-07-30 Architect：**新增 §连接与超时约定**——claim 事务与处理必须分离的硬约束（事务内禁止 LLM 调用）+ 四项超时取值（`statement_timeout` 30s / `idle_in_transaction_session_timeout` 60s / `lock_timeout` 5s / 建连 10s）+ 以 `ALTER ROLE ai_worker SET` 在数据库层强制、ai 侧零配置；**订正 §卡死回收机制三处错误**——扫描的是 `tasks.status='running'` 非 `processing`、未达上限回收到 `queued` 非 `retryable_failed`、**阈值默认 600s 而非契约此前写的 1800s（该数字无实现依据，ai 多轮引用的 1800s 源自此处，实际生效值 DevOps 已核实回填：test/prod `server/.env` 均 `600000`（600s），见 §卡死阈值）**。方案详见 xiaobao `docs/progress/ad-hoc/2026-07-30-spike-db-timeout-config.md`）。v1.6（2026-07-30 Architect：① `locked_by` 明确无格式约束、xiaobao 回收不读其内容、xiaobao 侧写入值为 `WORKER_ID`（默认 `worker-1`）请 ai 避开；② **新增「处理中」两表字面量差异专节**——`tasks.status='running'` vs `raw_items.l1_status='processing'`，并说明 `tasks` 无 CHECK 约束、写错 `processing` 会导致卡死回收永不触发；③ **新增 ai 自愈回收必须同步 `raw_items.l1_status` 的要求**，否则留下「前端显示解析中但任务在排队」的不一致；④ `sources.domain_tags` 类型定性——预期数组，`{}` 系 `schema.ts:67` 默认值误写（应为 `'[]'`）、语义等同未配置，附两侧归一化行为与数据实况。见 ai 2026-07-28 两帖）。v1.5（2026-07-28 Architect 答复 C-11~C-14：**撤回 v1.3 关于 `l0_label` 的错误结论**——`L1Input.domain_tags` 真源是 `sources.domain_tags`（源级静态标签，非 L0 产物），`l0_label` 是处理决策标记，两者语义无关；补 `l0_label` 完整取值域枚举；§sources 补 `domain_tags`/`attention_level` 行（权限待 GRANT）；明确 `priority` 方向为「数值大 = 优先」；退避越界取末值 + `max_attempts` 以列为准并登记 xiaobao 侧待订正的硬编码；标注 `source_item_url` 不保证协议前缀；登记 `processed_news.raw_item_id` 唯一约束为 ai 幂等前提。见 C-11/C-12/C-13/C-14）。v1.4（2026-07-27 权限矩阵照 DevOps 已执行 GRANT 补齐：`raw_items` SELECT + `source_item_url`/`l0_label`、`tasks` UPDATE + `run_after`，test + prod 双库对称，见 STATUS/沟通文档 GRANT 回帖）。v1.3（2026-07-27 Architect 事实订正：删除不存在的 `tasks.metadata` 列并补齐 `raw_item_id`/`run_after`/`max_attempts`/`priority`、补 `tasks.status` 枚举与时点对应表、claim 规则改为以 tasks 为准并补 `run_after` 退避条件、明确 `processed_news` 为「xiaobao 占位 INSERT + ai UPDATE」及 `id` 由 DB 生成、补 `published_at` 写入要求、标注 `score_total` 在 database 模式无触发点，见 C-2/C-3/C-4/C-5/C-8/C-9。权限矩阵变更已随 v1.4 落文档。v1.2 2026-07-27 订正 `tags_v2` 第五类笔误 `sentiment`→`processing` 对齐 HTTP 契约 + 明确 `language` 取值固定 `'zh'`，见 C-10 / C-7。v1.1 2026-07-25 订正 `score_total` 归属 + 合并 `l1_status` 枚举重复行，见 O-1 / O-5）
...
| 评分加权 score_total | ✅ 分工不变（读四维加权后写入，公式见 `l1-processor.ts:13` `calcScoreTotal`：`(T×0.25+I×0.35+C×0.25+X×0.15)×2` 保留 1 位）。**v1.9 已定补算方式**：database 模式由 xiaobao **轮询补算**（挂 worker tick，条件 `completed AND score_dimensions IS NOT NULL AND score_total IS NULL`，公式复用 `calcScoreTotal` 单一真源），实现随 ai v0.2 联调落地；落地前该列 NULL 属预期、排序 COALESCE 兜底 | ❌（只产 `score_dimensions` + reason，不算不写 `score_total`） |
...
| `needs_context` | boolean | **质量信号：上下文不足、结果存疑**（v1.9 新增，Q-1 定案；xiaobao 侧列迁移待落地——落地前 ai 写入会失败，ai v0.2 实现时以列存在为前置核对项；前端消费留后续迭代，先存数据）。**语义精确化（v1.11，ai Architect 实读 `graphs/news_l1.py:393-394` 核出、ai PM 2026-08-01 转达）**：该值 = **LLM 判断 OR ai 侧长度启发式**（原文 <300 字且非 raw 上下文 <500 字，`news_l1.py:294`）。判读须知两条：① `true` 有两个来源——LLM 判定证据不足 / 纯因原文过短，库内不区分来源；② **`false` 不代表「LLM 已确认证据充分」**——LLM 未输出该字段时默认 `False` 再与启发式做 `or`，原文够长即得 `false`，故「LLM 未表态」与「LLM 明确判定不需要」在本列同为 `false` 不可区分。ai v0.2 起在 LLM 未表态时于 `tags_v2.processing` 追加 **`degraded:needs_context_missing`** 留痕，消费方需区分「未表态的 `false`」时以该标签判别。**消费红线：`false` 只可当「未见存疑信号」用，不可当「证据充分」用**——前端角标、排序加权等一切消费均须按此口径 | needs_context |
| `score_total` | numeric | 综合分（**xiaobao 写入**：读四维加权计算，公式 `calcScoreTotal`。**database 模式补算方式已定（v1.9）：xiaobao 轮询补算**，挂 worker tick，条件 `completed AND score_dimensions IS NOT NULL AND score_total IS NULL`；实现随 ai v0.2 联调落地，此前该列为 NULL 属预期，排序由 COALESCE 兜底——联调判读须知照旧） | —（非 AI 输出） |
...
| `language` | varchar | **产出内容语种，固定 `'zh'`**（news-l1 输出契约即中文输出。占位行由 xiaobao 写 `'zh'`，ai UPDATE 保持即可；原文语种当前**不落任何列**——PM 2026-07-25 帖「归 raw_items.language」系笔误，该列不存在，v1.9 订正） | —（取值规则固定） |
...
| sources | SELECT 部分列 | id, type, identity, config, **domain_tags, attention_level**（v1.5 登记、DevOps 2026-07-28 双库已执行 verify；**v1.10 补录——此前矩阵漏更新**） |
...
### ⚠️ 授权粒度的非对称性 —— 「加一列」的后果因表而异（v1.10 新增）

**本表的三种授权粒度并存，导致 xiaobao 侧「给某表加一列」时 ai 的可见性完全不同**：

| 表 | 授权粒度 | xiaobao 加列后，ai 的状态 | 失败形态 |
|---|---|---|---|
| `processed_news` | **表级** `SELECT, INSERT, UPDATE` | **自动可写**，无需任何 GRANT 动作 | 无 |
| `raw_items` / `sources` / `tasks` | **列级** | **读不到 / 写不了新列** | **不报错、只是取不到值**——静默降级 |

**这条不是理论**：`sources.domain_tags` 就踩过一次。该列一直存在且是 `L1Input.domain_tags` 的真源，但因不在 ai 的列级授权内，ai 侧长期表现为「`domain_tags` 恒空」，双方各自排查了数轮才定位到是**授权缺列**而非数据问题（见 C-1 / C-14 / 6i 全过程）。

**两条纪律**：

1. **xiaobao 给 `raw_items` / `sources` / `tasks` 任一表加列、且该列对 ai 有意义时**，须**同时执行对应的列级 GRANT** 并在本矩阵登记——否则 ai 侧没有任何信号。
2. **本矩阵须与实际 GRANT 保持一致**。v1.10 补录 `sources` 的两列即为一次实例：GRANT 于 2026-07-28 执行、矩阵直到 08-01 才补上，中间 4 天里**文档所述权限窄于实际权限**。矩阵与实际不一致时，两个方向都有害——文档更宽会让 ai 误以为能读，文档更窄会让 ai 放弃本可使用的数据。

> 来源：ai DevOps 在 CN-009 确认时提出（`processed_news` 是表级、故「加 `needs_context` 列 ai 自动能写」正是 CN-009 成立的前提），并指出 `sources` 为列级、后果相反，交 ai Architect 判断是否入契约。**判断：应入**——它是 `domain_tags` 那次教训的一般化，且属「静默降级」这一最难被发现的类别。
