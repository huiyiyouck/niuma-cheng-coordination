...
| `needs_context` | boolean | **质量信号：上下文不足、结果存疑**（v1.9 新增，Q-1 定案；xiaobao 侧列迁移待落地——落地前 ai 写入会失败，ai v0.2 实现时以列存在为前置核对项；前端消费留后续迭代，先存数据） | needs_context |
