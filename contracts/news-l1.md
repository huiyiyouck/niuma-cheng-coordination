...
- 契约 id：`news-l1`
- 版本：**v1.1**（2026-08-01 新增 §服务端点与运行时坐标 —— 此前契约登记了「调什么」却未登记「调哪儿」，双侧端点与模式开关均无变更通知机制；ai 停用 8100 时对方无信号、对方改端口时 ai 亦无信号，且 AC-1.5 的有序回滚依赖双方知道对方当前模式。非 breaking，不改任何字段）。v1（2026-06-17 初版）
- 状态：生效中
...
> 上面的 `## Endpoint` 登记的是**路径**（调什么）。本节登记 **host:port 与模式开关**（调哪儿、当前处于哪种模式）——两者缺一，跨侧就无法独立确认「我该连谁、对方现在是什么状态」。
>
> **为什么不把端口写死在正文而单列一节**：端口是**环境相关的部署事实**，若混进字段定义，每次换端口都要升契约版本——高频、低价值，久了没人维护，反而退化成又一份过期文档。本节采用与 `AI_STALE_TIMEOUT_MS`（见 `news-l1-db.md` v1.8）相同的形态：**登记的重点不是「值不可变」，而是「它是契约项，任一侧变更须先改本节并通知对方」**。
...
**第 5/6 行的用途**：AC-1.5 的回滚是**有序的双侧动作**——先 xiaobao 切回集成模式、后 ai 切 `RUN_MODE`；反序会产生「两侧都不处理」的窗口。执行方须能读到对方当前模式，否则预案无法执行。

### 变更纪律（与 `AI_STALE_TIMEOUT_MS` 同级）

1. 变更上表任一项（含**停用某端点**）：**先改本节、再执行、并经 `communications/REQ-003-db-boundary-async.md` 知会对方**。
2. 停用端点尤其要通知——停用不会让对方报错，只会让对方在下次调用时才发现，且可能是很久以后。
3. 两侧的部署就绪检查须各含一条：**核对本节所列坐标与实际部署一致**。ai 侧已纳入其 A~F 判据。
